...
void Bot::InicializaVariables(){
    ...
    timerPeligro = IrrMngr::Inst()->getTimer();
    timePeligro = timerPeligro->getTime();
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
     ...
     Nodo* aux;
     Nodo* nodoInicial = getMas(m_gameObject.GetPosition().x, m_gameObject.GetPosition().y);
     Nodo* nodoDestino = getCercanoTotal(players.at(mandobusco)->GetPosition().x,players.at(mandobusco)->GetPosition().y);
     Nodo* nodoActual;
     ...
     if(nodoInicial->getPosicion().x == nodoDestino->getPosicion().x && nodoInicial->getPosicion().y == nodoDestino->getPosicion().y){
          pathfindingDist = new Lista();
          pathfindingDist->insertar(nodoInicial);
          nodoFinIni= pathfindingDist->getHead();
     }
           ...
           while(nodoActual!=NULL){
               pathfindingDist->insertar(nodoActual);
               nodoActual = nodoActual->getPadre();
           }

           nodoFinIni= pathfindingDist->getHead();
           //std::cout<<"<<<<<<<<<pathfindingDist>>>>>>>>>"<<std::endl;
           //pathfindingDist->imprimirLista();
           std::cout<<"Distancia hasta el jugador en nodos: "<<pathfindingDist->getTamanyo()<<std::endl;
           std::cout<<std::endl;
     }
...
void Bot::actualiza(){
    ...
    if(mando == 0)
      if(IrrMngr::Inst()->getTime()-timePeligro>1000){
        timerPeligro = IrrMngr::Inst()->getTimer();
        timePeligro = timerPeligro->getTime();
        getPeligro();
      }
...
void Bot::buscaArma(){
    ...
    if(cogiendo) estadoBot = 1;
        ...
        if(cogiendo) estadoBot = 1;
            ...
            std::cout<<"NodoINI: "<<nodoFinIni->getNumero()<<" NodoFin: "<<aux->getNumero()<<std::endl;
            calcularPathfinding(nodoFinIni,aux);
            if(pathfinding->getTamanyo()>1) pathfinding->remove(pathfinding->getUltimo()->getPosicion());
            //else saltar();
...
void Bot::buscaJugador(){
    ...
    if(mando != mandobusco && players.at(mandobusco)->getMuerto()==false
        ...
        std::cout<<"SOY BOT "<<mando<<" y BUSCO JUGADOR"<<std::endl;
...
void Bot::colisionConNodo(int nodo){
    if(pathfinding->getUltimo()->getNumero() == nodo ){
