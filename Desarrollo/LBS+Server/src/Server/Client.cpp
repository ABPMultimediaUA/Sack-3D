...
Client::Client():m_isServer(false),m_server(NULL){
...
void Client::iniciar(){
    ...
    if(!m_isServer)
        ...
        strncpy(auxclientPort, "1", sizeof(auxclientPort));
    ...
    if(server){
        ...
        while(!run){
            if(m_isServer && m_server)m_server->Update();
            recibir();
        }
    }
}
