...
void Pistola::usar(){
    if(usos){
        if(m_pIrrMngr->getTime()-timeCadencia > cadencia ){
            ...
            Particle *cap = m_pWorld->AddParticle(new Particle(new PBCap(),b2Vec2(m_gameObject.GetPosition().x/2.0f,-(m_gameObject.GetPosition().y/2.0f)),irr::core::vector3df(0.04f,.02f,.02f), irr::video::SColor(255,229, 229, 59)));
	    	b2Vec2 capVel;
	    	capVel.x = dir*rand()%3;
	    	capVel.y =rand()%3;
