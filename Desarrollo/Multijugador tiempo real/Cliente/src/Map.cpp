...
Map::Map(irr::core::stringw file){
     ...
     while (xml->read()){
          if(irr::core::stringw("objectgroup") == xml->getNodeName() && xml->getAttributeValue(L"name")){
               ...
               else if(irr::core::stringw("Muelle")     == xml->getAttributeValue(L"name")) capa = MUELLE;
               else if(irr::core::stringw("Teleport")   == xml->getAttributeValue(L"name")) capa = TELEPORT;
               ...
               if (xml->getNodeType() == irr::io::EXN_ELEMENT && irr::core::stringw("object") == xml->getNodeName()){
                    switch(capa){
                         case PLATAFORMA:{
                                   Platform(irr::core::vector3df(x+(width/2)-100,-1*(y+(height/2)),0),irr::core::vector3df(width, height, rand()%3+1),irr::video::SColor(255, rand()%255, rand()%255, rand()%255));
                         break;}
                         case TELEPORT:{
                                   int id = 0, partner = 0;
                                   do{
