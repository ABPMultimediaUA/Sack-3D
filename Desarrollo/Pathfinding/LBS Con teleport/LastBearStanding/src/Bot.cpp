...
void Bot::InicializaVariables(){
    ...
    teleports = World::Inst()->GetTeleports();
    Nodo* aux ;
...
void Bot::buscaJugador(){
    ...
    //Calculamos de nuevo el path cada dos segundos
    if(cogiendo && World::Inst()->getVivos() >1 && IrrMngr::Inst()->getTime()-timePeligro>2000){
        timerPeligro = IrrMngr::Inst()->getTimer();
        timePeligro = timerPeligro->getTime();
        ...
        nodoFinIni = getCercanoTotal(m_gameObject.GetPosition().x,m_gameObject.GetPosition().y);
...
void Bot::calcularPathfinding(Nodo* inicial, Nodo* objetivo){
     ...
     for(int i = 0; i<teleports.size(); i++){
            //std::cout<<"Posicion de destino X"<<(int)teleports.at(i)->getPosition().x<<std::endl;
            //std::cout<<"Posicion de destino Y"<<(-1*(int)teleports.at(i)->getPosition().y)+2<<std::endl;

        if((int)teleports.at(i)->getPosition().x == nodoDestino->getPosicion().x && ((-1*(int)teleports.at(i)->getPosition().y)+2) == nodoDestino->getPosicion().y){
            //std::cout<<"Este nodo es teleport y no vale!!!!!"<<std::endl;
            return;
        }
